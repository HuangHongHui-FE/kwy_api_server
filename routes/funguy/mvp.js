...
router.get("/kwy/tpq/mvp", async (req, res)=>{
	...
	try {
		...
		let data = await db.collection('image').find({"mask": "mvp"}).toArray()
        console.log(data)
		const dataList = []
		for(let i = 0; i < data[0].mvp.length; i++){
			// data[0].mvp[i]      读到的文件名
			let path = "static/tpq/fenlei/" + data[0].mvp[i]
			pReadFile(path).then(function(data2){
				// 将数据转换成base64
				data2 = data2.toString('base64')
				dataList.push({data: data2, title: data[0].mvp[i]})
				if(i === data[0].mvp.length - 1){
